...
noiseLevel(:,2) = [1,0,0]; %sensor noise only
noiseLevel(:,3) = [0,0.001,0.0005]; %movement noise only
noiseLevel(:,4) = [1,0.001,0.0005]; %all the noise
...
startPositions =  [20,10;20,20;30,20;50,70 ]; %These will change
targetPositions = [50,30;80,80;100,20;230,70]; %These will change
...
averageCompletionTime = sum(sum(sum(resultsTime,3)/numberOfrepeats,2)/size(noiseLevel,2))/size(maps,1);
averageDis = sum(sum(sum(resultsDis,3)/numberOfrepeats,2)/size(noiseLevel,2))/size(maps,1);
averageLength = sum(sum(sum(resultsLength,3)/numberOfrepeats,2)/size(noiseLevel,2))/size(maps,1);
percentCollision = 100*sum(sum(sum(resultsCollision)))/(numberOfrepeats*size(noiseLevel,2)* size(maps,1));
disp('Average values');
fprintf('Time: %.3f, Distance: %.3f, Length: %.3f, Collision%%: %.1f\n',averageCompletionTime,averageDis,averageLength,percentCollision);