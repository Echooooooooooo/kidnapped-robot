...
path_div = 5; % Define number of divisions the max distance is split into
...
if mapArea <= 1600
    ...
    unique_clusters = round(-0.0002*mapArea + 9.4);
end

wallClearance = 5; % wall clearance of bot
...
while(converged == 0 && n < maxNumOfIterations) %particle filter loop
    ...
    if size(C) <= unique_clusters %if the number of clusters is below a certain value = converged
        ...
        converged = 1;
        %disp('Convergence - Returning Bot Position Estimate')
    else
...
if n == maxNumOfIterations
    if size(C) <= 1.4*unique_clusters
        ...
        disp('Unsure of position - relocalising')
    end
end
end
